...
function isMuted(pm2Name: string): boolean {
  ...
  const rows = db
    .select()
    .from(alertPrefs)
    .where(eq(alertPrefs.pm2Name, pm2Name))
    .all();
...
function substitute(template: string, data: Record<string, string>): string {
  ...
  return template
    .replace(/\{logLevel\}/g, get("logLevel"))
    .replace(/\{log_message\}/g, get("log_message"))
    .replace(/\{process_name\}/g, get("process_name"));
}
  ...
  eventBus.on(
    ...
    (data: { text: string; processName: string; level: string }) => {
      try {
        ...
        const throttleMin = parseInt(
          getSetting("alert.throttleMinutes", "60"),
          10,
        );
        ...
        if (whEnabled === "1") {
          ...
          try {
            whHeaders = JSON.parse(
              getSetting("reporter.webhook.headers", "[]"),
            );
          } catch {}
          try {
            whBody = JSON.parse(getSetting("reporter.webhook.body", "[]"));
          } catch {}
          if (whUrl) {
