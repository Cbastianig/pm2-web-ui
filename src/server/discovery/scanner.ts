...
export function scanAll(scanPath: string): {
  apps: DiscoveredApp[];
  unconfigured: UnconfiguredApp[];
} {
  const now = Date.now();
  ...
  try {
    if (!fs.existsSync(scanPath)) {
      console.warn(`[OPS] Scan path does not exist: ${scanPath}`);
      cachedApps = apps;
    ...
    if (!fs.statSync(scanPath).isDirectory()) {
      console.warn(`[OPS] Path is not a directory: ${scanPath}`);
      cachedApps = apps;
      cachedUnconfigured = unconfigured;
      lastScan = now;
      return { apps, unconfigured };
